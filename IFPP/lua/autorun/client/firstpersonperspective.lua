CreateClientConVar("cam", "0", true, true)
...
local function FirstPersonPerspective(ply, pos, angles, fov)
	...
	if GetConVarNumber("cam") < 1 or not ply:Alive() or (traceHit and not ply:InVehicle()) or not eyeAtt then
		return
	end
...
function IFPP_DotCrosshair()
	...
	if GetConVarNumber("cam") < 1 or not ply:Alive() or traceHit then
		return
	end
...
function IFPP_Think()
	...
	if GetConVarNumber("cam") < 1 or not ply:Alive() then
		return
	end
	
	ply.BuildBonePositions = function(ply, numbon, numphysbon)
		if GetConVarNumber("cam") < 1 then
			return
		end
...
function IFPP_SetupMove(ucmd)
	if GetConVarNumber("cam") <= 0 or not LocalPlayer():Alive() then
		return
	end
